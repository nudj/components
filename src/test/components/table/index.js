...
describe('Table', () => {
  describe('data', () => {
    ...
    it('renders the provided title as the column title', () => {
      const component = shallow(<Table data={data} columns={columns} />)
      const header = (index) => component.find('defaultHeading').at(index).dive().props()
  ...
  describe('component overrides', () => {
    ...
    it('has a table head component that can be overridden', () => {
      ...
      const component = shallow(<Table data={data} columns={columns} />)
      const customComponent = shallow(<Table tableHead={customHead} data={data} columns={columns} />)
      expect(component.find('defaultTable').children().first().name()).to.equal('defaultHead')
      expect(customComponent.find('defaultTable').children().first().name()).to.equal('customHead')
    })

    it('has a header row component that can be overridden', () => {
      const customHeaderRow = (props) => <tr>{props.children}</tr>
      const component = shallow(<Table data={data} columns={columns} />)
      const customComponent = shallow(<Table headerRow={customHeaderRow} data={data} columns={columns} />)
      expect(component.find('defaultHead').children().first().name()).to.equal('defaultHeaderRow')
      expect(customComponent.find('defaultHead').children().first().name()).to.equal('customHeaderRow')
    })
  ...
  describe('styles', () => {
    ...
    it('can be given a general header-row class', () => {
      ...
      const props = component.find('defaultHead').children().props()
    ...
    it('can be given a general heading/title class', () => {
      ...
      const props = component.find('defaultHeading').at(0).dive().props()
